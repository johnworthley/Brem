...
class Signin extends Component {
  ...
  handleLogin(e) {
    e.preventDefault()
    const { loginRef, passwordRef } = this
    ...
    fetch('https://guitclub.ru/api/signin', {
      method: 'POST',
      credentials: 'include',
      body: JSON.stringify({
        login,
    ...
    .then(data => {
      ...
      store.update({username: login, userstatus: status})
      this.setState({successfulSignup: true})
      setTimeout(() => {
        this.setState({needToRedirect: true})
        store.disableFullscreen('signup')
      }, 1500)
    })
    .catch(e => {
      this.setState({
  ...
  render() {
    const bindRef = name => el => this[name] = el
    ...
    return (
      <div style={mainHolderStyle}>
        ...
        <form style={mainFormStyle} action="">
          ...
          <input ref={bindRef('loginRef')} style={{...formItemInput}} type="text"/>
          <div style={{...formItem}}>Ваш пароль</div>
          <input ref={bindRef('passwordRef')} style={{...formItemInput}} type="text"/>
          <button onClick={this.handleLogin.bind(this)} style={{...formButton}}>Войти</button>
          <span ref={bindRef('errorRef')} style={{display: 'inline', marginTop: 9, backgroundColor: '#fff', color: '#c10f0f', transition: 'opacity .3s ease', opacity: error ? 1 : 0}}>
